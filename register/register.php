	...
	if ($expecting($_POST,array($email,$pwd,$pwdConfirm,$name,$phone))) {
		...
		if ($errors === "") {
			...
			if ($sql->connect_errno) {
				echo format_error_message("<br />MySQL error ".$sql->connect_errno.": ".$sql->connect_error);
			} else {
				...
				if ($errors === "") {
					...
					echo "<p>Registration successful! <a href='../login'>Click here</a> to log in.</p>";
				} else {
					echo format_error_message($errors);
				}
				$sql->close();
			}
		} else {
		...
		echo "HTTP 403: You do not have permission to access this page.";
	}
?>